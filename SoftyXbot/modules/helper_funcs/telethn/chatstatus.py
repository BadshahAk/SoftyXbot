...
from SoftyXbot import DRAGONS
from SoftyXbot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Softy_is_admin(chat_id: int):
    status = False
    Softy = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Softy.id == user.id:
