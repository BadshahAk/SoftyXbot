...
from SoftyXbot import BOT_ID
from SoftyXbot import DRAGONS as DEVS
from SoftyXbot import OWNER_ID
from SoftyXbot import telethn as Softy
from SoftyXbot.events import Softyinline
from SoftyXbot.events import register as Softybot
from SoftyXbot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Softy(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Softy(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Softybot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
*ғᴏʀᴄᴇ ꜱᴜʙꜱᴄʀɪʙᴇ:*
   ...
   •➥ *ꜱᴇᴛᴜᴘ*
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/SoftyXbot?startgroup=new)
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ your ᴄʜᴀɴɴᴇʟ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/SoftyXbot?startgroup=new)
